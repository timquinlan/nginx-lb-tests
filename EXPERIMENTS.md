...
**Not yet built: a backend-side concurrency cap.** `backend/server.py` uses `ThreadingHTTPServer`, which spawns one unbounded OS thread per connection -- there is currently no concurrency limit to turn down. The plan (discussed 2026-07-29, not yet implemented):

- A small resizable gate in the backend (a counter + condition variable, since `threading.Semaphore` can't be resized in place) capping concurrent in-flight requests per backend.
- A lightweight admin endpoint (e.g. `POST /admin/capacity {"limit": N}`) called directly against each backend (bypassing NGINX, same pattern as `sampler.py`'s `direct_probe()`), so the cap is **live-adjustable from the traffic generator** rather than requiring a container restart the way `LATENCY_MIN_MS`/`DEGRADATION_*` do. Those other knobs stay restart-based on purpose -- see "What NOT to move into the generator" below.
- `traffic_generator.py` gets a `--contention {off,mild,moderate}` flag that computes the right N from the run's *actual* `--rps` (see math below) and pushes it to all five backends before sending traffic. `off` preserves today's unlimited behavior as the default -- nothing changes for existing runs.
