...
`aco_lc`'s spot at the top is now backed by 10 independent runs across two axes (rps, ACO tuning) -- including a from-scratch reproduction of the full rps ladder on a separately rebuilt container (see "`aco_lc`: the clean winner" below) -- with zero exceptions across all 50 stat-comparisons. It hasn't been run through the multi-instance/contention and overload battery the rest of this hierarchy survived, but within the axes actually tested, it's had at least as much reproducibility scrutiny as anything else in this document.
...
**First batch: six runs, two independently varied axes, zero exceptions** (a second, independent batch follows below). Four back-to-back runs at increasing rps (40/80/160/320, 10-minute runs, 10s tick, tuned default evaporation 0.3/deposit 10 -- the only four rps points where the traffic generator actually delivered the requested load, see the throughput-ceiling caveat below for why 640/1280 are excluded):
...
**Independently reproduced after the project-wide algorithm rename** (`combo` -> `aco_lc`, see `AGENT.md`), the same rps ladder was re-run from scratch -- fresh container build, fresh pheromone state, purged logs, run indices back to 1 -- as a genuine independent check rather than a re-analysis of the same data:

| rps | mean gap | median gap | p90 | p95 | p99 |
|---|---|---|---|---|---|
| 40 | -25.3ms (-6.9%) sig. | -30.0ms (-8.5%) sig. | -39.0ms (-6.9%) sig. | -33.0ms (-5.2%) sig. | -29.0ms (-3.8%) sig. |
| 80 | -25.4ms (-6.9%) sig. | -22.0ms (-6.3%) sig. | -49.0ms (-8.6%) sig. | -48.0ms (-7.6%) sig. | -24.0ms (-3.3%) sig. |
| 160 | -33.3ms (-9.1%) sig. | -25.0ms (-7.4%) sig. | -74.0ms (-12.6%) sig. | -64.0ms (-9.7%) sig. | -40.0ms (-5.2%) sig. |
| 320 | -34.9ms (-9.7%) sig. | -45.0ms (-13.5%) sig. | -39.0ms (-6.9%) sig. | -35.0ms (-5.7%) sig. | -33.0ms (-4.7%) sig. |

**50 of 50 stat-comparisons across all 10 runs (two separate rps ladders, two separate container builds, plus the two tuning-variant runs) are significant, every one in `aco_lc`'s favor -- no exceptions, ever.** This is the clean winner: the first and only mechanism in this project that beats `leastconn` outright, on every stat, rather than narrowing the gap (tuned `aco_wrr`) or tying on tail latency alone. Typically 20-35ms/5-14% faster than `leastconn` on mean TTFB across both ladders, with the tail-latency wins (p90 especially) often larger in percentage terms than the mean win.

**Scope, stated plainly.** This result rests on one axis-pair (rps, ACO tuning) against the project's current default configuration -- it has not been run through the multi-instance/contention battery the rest of this document's headline ranking survived, and (per current project scope) isn't planned to be against the tick/backend-count/latency-profile axes either. Within the axes actually tested, though, this is no longer a single sweep's result: it's been independently reproduced end-to-end, including across a full rename and container rebuild, at a reproducibility bar at least as strong as `aco_wrr`'s own tuning result got before being treated as settled above.
...
**The best-performing mechanism found in this project is `aco_lc`** (`least_conn` plus ACO-set weights, see above) -- a clean, significant win over `leastconn` on every stat (mean/median/p90/p95/p99), across every condition tried so far: 10 independent runs, 2 varied axes (rps 40-320, ACO evaporation 0.15-0.5), including a full from-scratch reproduction after a project-wide rename and container rebuild -- 50 of 50 stat-comparisons significant, zero exceptions in either direction, ever. It doesn't out-react `leastconn`'s zero-lag signal the way `aco_wrr`/`mc_wrr` try and fail to -- it hands that same live signal a second, ACO-derived input to break ties with, and that combination beats plain `leastconn` outright rather than just narrowing the gap on it.
...
`aco_lc`'s result still covers fewer axes than everything below it in this hierarchy -- rps and ACO tuning only, not the full multi-instance/contention battery the rest of this document survived, and (per current project scope) not the tick/backend-count/latency-profile axes either (see "`aco_lc`: the clean winner" above for the exact scope). But within that scope it's no longer a single sweep's result -- it's been independently reproduced end-to-end, at a reproducibility bar at least as strong as anything else in this document. `mc_lc` also exists now (see `AGENT.md`) but hasn't yet gone through any of this project's reproducibility/scope scrutiny -- not included in the "clean winner" claim above pending its own runs.
