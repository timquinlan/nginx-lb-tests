...
- **Random** (`/random`) -- NGINX's `random` upstream directive: a fresh weighted dice roll per request, no memory of past selections at all.
...
**Validated live, all six paths simultaneously, at every default along the way:** 60s smoke tests and full 10-minute runs at both 125 and 250 completed with zero `worker_connections` warnings or errors in any `*.error.log`; nearly every stat (mean/median/p90/p95) for every non-control algorithm came back statistically significant at `alpha=0.05` within 10 minutes at 125 already, so 250 and 500 only add more headroom, not more required duration. A full 10-minute run at the 500 default itself was in progress as this was written -- the CPU/error figures above are from live spot-checks during that run (`docker stats` sampled mid-run), not yet the completed run's own analysis; see `AGENT.md`, "Phase 6," for whichever is more current by the time you're reading this. The single-worker `~650rps/path` ceiling below still predates both the six-path config and the multi-worker move -- not yet independently re-measured for exactly where *this* config's own ceiling sits, only that 500rps/path (the current default) is comfortably under it.
