...
- **`./logs/analysis/`** -- Phase 4/5's output for each analyzed run: PNG charts (TTFB over time with a degradation overlay, per-backend selection frequency) and `runN_stats_report.txt` (TTFB stats, ranking, and significance testing for every discovered algorithm). See `analysis/README.md` for the full field-by-field breakdown of what's in the report.
...
Each run automatically triggers analysis at the end -- a stdout summary table, PNG charts, and a text stats report, all under `./logs/analysis/` (see "Data & where it lands" above, and `analysis/README.md` for the full report format). To analyze a run again later, or a different run than the most recent one:
...
On a Mac M4, `--rps 320` is the highest value confirmed to actually deliver what it asks for: runs at 40/80/160/320 all landed within ~0.2% of their target request count, but a 640 run only achieved ~551rps/path and a 1280 run landed at essentially the same ~548rps/path -- doubling the target didn't move the achieved rate, the signature of a hard ceiling (this generator sizes `0.5 threads/rps/path`, so 640rps across the current 6-path roster means ~1,920 OS threads in one process -- GIL/OS-scheduling contention, not backend or NGINX behavior; this was originally measured at 7 paths/~2,240 threads before a later rename dropped the roster to 6, same order of magnitude and conclusion either way). No `worker_connections` warning fires when this happens -- it fails silently, so **check actual request counts (`n` in the stats report) against `rps x duration` for anything above ~320rps/path on comparable hardware** rather than trusting the requested rate was what actually ran. See `FINDINGS.md`'s caveats section for the full data.
